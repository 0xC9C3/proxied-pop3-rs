// all credits to https://github.com/mattnenterprise/rust-pop3
...
impl POP3Stream {
    ...
    pub fn connect<A:ToSocketAddrs>(addr: A,ssl_context: Option<SslConnector>,domain: &str) -> Result<POP3Stream> {
    ...
    pub fn connect_using_socket(
        ...
        let mut socket = match ssl_context {
            Some(context) => POP3Stream {
                stream: Ssl(SslConnector::connect(&context, domain, stream).unwrap()),
                is_authenticated: false},
            None => POP3Stream {
                stream: Basic(stream),
                is_authenticated: false},
